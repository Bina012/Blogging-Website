    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            ...
            <div class="card-toolbar">
                <a href="{{ url('cd-admin/add-blogcategory') }}">
                    
                    <button type="submit" class="btn btn-sm btn-success font-weight-bold">
                        <i class="flaticon2-add"></i> Add blog category
                    </button>
                </a>
            </div>
        </div>
        <div class="card-body">
            ...
            <table class="table table-bordered " id="#">
                ...
                <tbody>
                   @if (count($blogCategorys))
                   @foreach ($blogCategorys as $blogcategory)
                   <tr>
                      <td>{{ $loop->iteration }}</td>
                      <td>{{ $blogcategory->category_name }}</td>
                      <td><img src='{{ url('uploads/images/blogcategory/' . $blogcategory->image) }}' height="50px"
                        width="50px">
                      
                      <td>
                        <span>
                            <a href="{{ route('blogcategory.view', ['id' => $blogcategory->id]) }}"
                                class="btn btn-sm btn-clean btn-icon btn-icon-md" title="view"><i
                                    class="la la-eye"></i></a>
                        </span>
                        <span style="display: inline-block;">
                            <a href="{{ route('blogcategory.edit', ['id' => $blogcategory->id]) }}"
                    ...
                    @else
                        <tr>
                            <td colspan="4">No Blog category Found</td>
                        </tr>
                    @endif
                </tbody>
            </table>
